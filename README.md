...
Phase 0:
Film video of a specular non transparent object, using sunlight prefereably advoid shadow if possible
A sample video or dataset that we know definitely is good enough for this 
...
Logic: Calculate Laplacian Variance for each frame. If variance < (?some number) (blurry), discard it.
[ ] 1.3 [OPTIONAL] AI Background Removal:
Teacher Note: Backgrounds actually HELP feature matching. Only use this if the reconstruction fails due to moving background objects.
...
Logic: Use subprocess to call colmap feature_extractor. (Point it directly to the images/ folder. Only pass the mask path if we ended up needing Task 1.3).
[ ] 2.3 Matcher Script:
...
Logic: Call colmap mapper. This generates the camera poses and sparse points.
[ ] 2.5 Export Data:
...
Logic: Run colmap image_undistorter, colmap patch_match_stereo, and colmap stereo_fusion.
...
Logic: Run o3d.geometry.TriangleMesh.create_from_point_cloud_poisson.
Logic: Project the colors from the dense point cloud onto the new mesh vertices.
